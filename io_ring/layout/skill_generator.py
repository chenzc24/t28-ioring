...
class SkillGeneratorT28:
    ...
    def generate_digital_io_features_with_inner(self, outer_pads: List[dict], inner_pads: List[dict], ring_config: dict) -> List[str]:
        ...
        # ----------------------------------------------------------------
        # Group pads by domain so each digital domain gets its own isolated
        # config wires. This prevents DVDD2A/B/C/D from being shorted.
        # ----------------------------------------------------------------
        from collections import defaultdict
        domain_pads: dict = defaultdict(list)
        for pad in all_digital_pads:
            domain_pads[pad.get("domain", "_ungrouped")].append(pad)

        for domain_id, pads_in_domain in domain_pads.items():
            # Group this domain's pads by orientation
            oriented_pads = {"R0": [], "R90": [], "R180": [], "R270": []}
            for pad in pads_in_domain:
                oriented_pads[pad["orientation"]].append(pad["position"])

            # --- Generate config wire segments per orientation ---
            side_endpoints: dict = {}
            ...
            # --- Corner connections within this domain only ---
            corner_connections = {
                "top_left":    ("R180", "R270"),
                "top_right":   ("R180", "R90"),
                "bottom_left": ("R0",   "R270"),
            ...
            for corner_name, (s1, s2) in corner_connections.items():
                if s1 not in side_endpoints or s2 not in side_endpoints:
                    continue

                if corner_name == "top_left":
                    x1,y1 = side_endpoints["R180"]["high"]["x_range"][0], side_endpoints["R180"]["high"]["y"]
                    x2,y2 = side_endpoints["R270"]["high"]["x"],          side_endpoints["R270"]["high"]["y_range"][1]
                    x1l,y1l = side_endpoints["R180"]["low"]["x_range"][0], side_endpoints["R180"]["low"]["y"]
                    x2l,y2l = side_endpoints["R270"]["low"]["x"],          side_endpoints["R270"]["low"]["y_range"][1]
                elif corner_name == "top_right":
                    x1,y1 = side_endpoints["R180"]["high"]["x_range"][1], side_endpoints["R180"]["high"]["y"]
                    x2,y2 = side_endpoints["R90"]["high"]["x"],           side_endpoints["R90"]["high"]["y_range"][1]
                    x1l,y1l = side_endpoints["R180"]["low"]["x_range"][1], side_endpoints["R180"]["low"]["y"]
                    x2l,y2l = side_endpoints["R90"]["low"]["x"],           side_endpoints["R90"]["low"]["y_range"][1]
                elif corner_name == "bottom_left":
                    x1,y1 = side_endpoints["R0"]["high"]["x_range"][0],  side_endpoints["R0"]["high"]["y"]
                    x2,y2 = side_endpoints["R270"]["high"]["x"],          side_endpoints["R270"]["high"]["y_range"][0]
                    x1l,y1l = side_endpoints["R0"]["low"]["x_range"][0],  side_endpoints["R0"]["low"]["y"]
                    x2l,y2l = side_endpoints["R270"]["low"]["x"],          side_endpoints["R270"]["low"]["y_range"][0]
                elif corner_name == "bottom_right":
                    x1,y1 = side_endpoints["R0"]["high"]["x_range"][1],  side_endpoints["R0"]["high"]["y"]
                    x2,y2 = side_endpoints["R90"]["high"]["x"],           side_endpoints["R90"]["high"]["y_range"][0]
                    x1l,y1l = side_endpoints["R0"]["low"]["x_range"][1],  side_endpoints["R0"]["low"]["y"]
                    x2l,y2l = side_endpoints["R90"]["low"]["x"],           side_endpoints["R90"]["low"]["y_range"][0]
